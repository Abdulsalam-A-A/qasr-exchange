...
const INIT = [
  { id: 1, code: "USD", name: "الدولار الأمريكي",   flag: "🇺🇸", buy: 153800, sell: 154200, change: 0, note: "لكل 100$" },
  { id: 2, code: "EUR", name: "اليورو",              flag: "🇪🇺", buy: 115,    sell: 116,    change: 0, note: "لكل 100$" },
  { id: 3, code: "GBP", name: "الجنيه الإسترليني",  flag: "🇬🇧", buy: 130,    sell: 133,    change: 0, note: "لكل 100$" },
  { id: 4, code: "TRY", name: "الليرة التركية",     flag: "🇹🇷", buy: 4500,   sell: 4300,   change: 0, note: "لكل 100$" },
  { id: 5, code: "JOD", name: "الدينار الأردني",    flag: "🇯🇴", buy: 71,     sell: 70,     change: 0, note: "لكل 100$" },
  { id: 6, code: "AED", name: "الدرهم الإماراتي",   flag: "🇦🇪", buy: 385,    sell: 367,    change: 0, note: "لكل 100$" },
  { id: 7, code: "SAR", name: "الريال السعودي",      flag: "🇸🇦", buy: 385,    sell: 375,    change: 0, note: "لكل 100$" },
  { id: 8, code: "CAD", name: "الدولار الكندي",     flag: "🇨🇦", buy: 68,     sell: 68,     change: 0, note: "100 كندي = 68$" },
  { id: 9, code: "AUD", name: "الدولار الأسترالي",  flag: "🇦🇺", buy: 67,     sell: 67,     change: 0, note: "100 أسترالي = 67$" },
];
...
function AdminPage({ currencies, setCurrencies, lastUpdated, setLastUpdated, onLogout }) {
  const [editRow, setEditRow] = useState(null);
  const [showAdd, setShowAdd] = useState(false);
  const [addForm, setAddForm] = useState({ flag:"", code:"", name:"", buy:"", sell:"", change:"", note:"" });
  const [flash, setFlash] = useState(false);

  function trigFlash() { setFlash(true); setTimeout(() => setFlash(false), 2000); }

  function saveEdit() {
    setCurrencies(prev => prev.map(c => c.id === editRow.id
      ? { ...c, buy:parseFloat(editRow.buy), sell:parseFloat(editRow.sell), change:parseFloat(editRow.change) }
      : c));
    setLastUpdated(new Date()); setEditRow(null); trigFlash();
  }

  function addCurrency(e) {
    e.preventDefault();
    setCurrencies(prev => [...prev, {
      id: Date.now(), flag: addForm.flag||"🏳️",
      code: addForm.code.toUpperCase(), name: addForm.name,
      buy: parseFloat(addForm.buy), sell: parseFloat(addForm.sell),
      change: parseFloat(addForm.change)||0, note: addForm.note||"",
    }]);
    setAddForm({ flag:"", code:"", name:"", buy:"", sell:"", change:"", note:"" });
    setShowAdd(false); setLastUpdated(new Date()); trigFlash();
  }
    ...
    <div style={S.root}>
      ...
      <header style={S.header}>
        <div style={S.hInner}>
          ...
          <div style={{ display:"flex", gap:10, alignItems:"center" }}>
            {flash && <span style={S.flashBadge}>✅ تم الحفظ!</span>}
      ...
      <main style={{ ...S.main, paddingTop:24 }}>
        <p style={{ color:"#64748b", fontSize:13, marginBottom:16 }}>
          آخر نشر: <strong style={{ color:"#f0c040" }}>{fmtDate(lastUpdated)} — {fmtTime(lastUpdated)}</strong>
        </p>
        {showAdd && (
          <form onSubmit={addCurrency} style={S.addForm}>
              ...
              {[["رمز العلم","flag","🇺🇸"],["الرمز","code","USD"],["الاسم بالعربي","name","الدولار الأمريكي"],
                ...
                <div key={key}>
                  ...
                  <input style={S.addInput} placeholder={ph} value={addForm[key]}
                    onChange={e => setAddForm(f=>({...f,[key]:e.target.value}))} required={["flag","code","name","buy","sell"].includes(key)}/>
                </div>
              ))}
            </div>
                ...
                <tr key={c.id} style={{...S.tr, background:i%2===0?"rgba(255,255,255,0.03)":"transparent"}}>
                  ...
                  {editRow?.id===c.id ? <>
                    ...
                    <td style={S.td}>
                      ...
                      <button onClick={()=>setCurrencies(p=>p.filter(x=>x.id!==c.id))} style={{...S.deleteBtn,marginRight:6}}>حذف</button>
                    </td>
                  </>}
                </tr>
              ))}
...
function PublicPage({ currencies, lastUpdated, onAdminClick, now, tick }) {
  const offset = (tick * 0.35) % (currencies.length * 240);
  return (
...
export default function App() {
  ...
  if (page==="admin") return <AdminPage currencies={currencies} setCurrencies={setCurrencies} lastUpdated={lastUpdated} setLastUpdated={setLastUpdated} onLogout={()=>setPage("public")}/>;
  return <PublicPage currencies={currencies} lastUpdated={lastUpdated} onAdminClick={()=>setPage("login")} now={now} tick={tick}/>;
}

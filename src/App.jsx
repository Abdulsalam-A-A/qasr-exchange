...
async function fetchLastUpdated() {
  const res = await fetch(`${API}?select=updated_at&order=updated_at.desc&limit=1`, { headers: HEADERS });
...
async function fetchHistory(code, days) {
  ...
  const res = await fetch(
    `${SUPABASE_URL}/rest/v1/price_history?currency_code=eq.${code}&recorded_at=gte.${fromStr}&order=recorded_at.asc`,
    { headers: HEADERS }
  );
  return res.json();
}
...
function RateGraph({ currencies, onClose }) {
  ...
  useEffect(() => { loadHistory(); }, [selected, period]);

  async function loadHistory() {
    setLoading(true);
    const data = await fetchHistory(selected, period);
    setHistory(data);
    setLoading(false);
  }
  ...
  const vals = history.map(h => view === "buy" ? h.buy : h.sell);
  const labels = history.map(h => {
    const d = new Date(h.recorded_at);
    return `${d.getDate()}/${d.getMonth()+1}`;
  });

  const minVal = vals.length ? Math.min(...vals) * 0.999 : 0;
  const maxVal = vals.length ? Math.max(...vals) * 1.001 : 1;
  const W = 600, H = 220, PAD = 40;

  function toX(i) { return PAD + (i / Math.max(vals.length-1,1)) * (W - PAD*2); }
  function toY(v) { return H - PAD - ((v - minVal) / (maxVal - minVal)) * (H - PAD*2); }

  const points = vals.map((v,i) => `${toX(i)},${toY(v)}`).join(" ");
  const fillPoints = vals.length
    ? `${toX(0)},${H-PAD} ` + vals.map((v,i) => `${toX(i)},${toY(v)}`).join(" ") + ` ${toX(vals.length-1)},${H-PAD}`
    : "";
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.75)", zIndex:500, display:"flex", alignItems:"center", justifyContent:"center", padding:16 }}>
      <div style={{ background:"#0d1526", border:"1px solid rgba(240,192,64,0.25)", borderRadius:16, width:"100%", maxWidth:720, padding:24, position:"relative" }}>
        {/* Close */}
        <button onClick={onClose} style={{ position:"absolute", top:16, left:16, background:"rgba(255,255,255,0.05)", border:"none", color:"#94a3b8", borderRadius:8, padding:"4px 10px", cursor:"pointer", fontSize:18 }}>✕</button>

        <h3 style={{ color:"#f0c040", textAlign:"center", fontFamily:"Georgia,serif", marginTop:0, marginBottom:20 }}>الرسم البياني للأسعار</h3>

        {/* Currency selector */}
        <div style={{ display:"flex", gap:8, flexWrap:"wrap", justifyContent:"center", marginBottom:16 }}>
          {currencies.map(c => (
            <button key={c.code} onClick={() => setSelected(c.code)}
              style={{ background: selected===c.code ? "#f0c040" : "rgba(255,255,255,0.05)",
                color: selected===c.code ? "#0a0f1e" : "#94a3b8",
                border:"1px solid rgba(240,192,64,0.2)", borderRadius:6,
        ...
        <div style={{ display:"flex", gap:8, justifyContent:"center", marginBottom:16 }}>
          {["buy","sell"].map(t => (
            <button key={t} onClick={() => setView(t)}
              style={{ background: view===t ? (t==="buy"?"#4ade80":"#60a5fa") : "rgba(255,255,255,0.05)",
                color: view===t ? "#0a0f1e" : "#94a3b8",
                border:"none", borderRadius:6, padding:"6px 20px", cursor:"pointer", fontSize:13, fontWeight:700 }}>
              {t==="buy" ? "شراء" : "بيع"}
            </button>
          ))}
        </div>
        ...
        <div style={{ display:"flex", gap:6, justifyContent:"center", marginBottom:20, flexWrap:"wrap" }}>
          {periods.map(p => (
            <button key={p.days} onClick={() => setPeriod(p.days)}
              style={{ background: period===p.days ? "rgba(240,192,64,0.2)" : "rgba(255,255,255,0.04)",
                color: period===p.days ? "#f0c040" : "#64748b",
                border: period===p.days ? "1px solid rgba(240,192,64,0.4)" : "1px solid rgba(255,255,255,0.08)",
        ...
        {cur && (
          <div style={{ display:"flex", justifyContent:"center", gap:24, marginBottom:16 }}>
            <div style={{ textAlign:"center" }}>
              <div style={{ color:"#64748b", fontSize:11 }}>سعر الشراء</div>
              <div style={{ color:"#4ade80", fontWeight:700, fontSize:18, fontFamily:"monospace" }}>{Number(cur.buy).toLocaleString("en")}</div>
            </div>
            <div style={{ textAlign:"center" }}>
              <div style={{ color:"#64748b", fontSize:11 }}>سعر البيع</div>
              <div style={{ color:"#60a5fa", fontWeight:700, fontSize:18, fontFamily:"monospace" }}>{Number(cur.sell).toLocaleString("en")}</div>
            </div>
            <div style={{ textAlign:"center" }}>
              <div style={{ color:"#64748b", fontSize:11 }}>الوحدة</div>
              <div style={{ color:"#f0c040", fontSize:13 }}>{cur.note||"—"}</div>
            </div>
          </div>
        )}

        {/* Chart */}
        <div style={{ background:"rgba(255,255,255,0.02)", borderRadius:12, padding:8, border:"1px solid rgba(255,255,255,0.06)" }}>
          {loading ? (
            <div style={{ height:220, display:"flex", alignItems:"center", justifyContent:"center", color:"#64748b" }}>جاري التحميل...</div>
          ) : vals.length < 2 ? (
            <div style={{ height:220, display:"flex", alignItems:"center", justifyContent:"center", color:"#64748b", fontSize:13 }}>
              لا توجد بيانات كافية لهذه الفترة — حدّث الأسعار يومياً لتراكم البيانات
            </div>
          ) : (
              ...
              {[0,0.25,0.5,0.75,1].map(t => {
                ...
                return (
                  <g key={t}>
                    ...
                    <text x={PAD-4} y={y+4} textAnchor="end" fontSize="10" fill="#475569">{Math.round(val).toLocaleString("en")}</text>
                  </g>
                );
              })}
              {/* Date labels - show 5 evenly */}
              {[0,0.25,0.5,0.75,1].map(t => {
                const i = Math.floor(t*(labels.length-1));
                const x = toX(i);
                return <text key={t} x={x} y={H-PAD+16} textAnchor="middle" fontSize="10" fill="#475569">{labels[i]}</text>;
              })}
              {/* Fill */}
              <polygon points={fillPoints}
                fill={view==="buy" ? "rgba(74,222,128,0.08)" : "rgba(96,165,250,0.08)"}/>
              {/* Line */}
              <polyline points={points} fill="none"
                stroke={view==="buy" ? "#4ade80" : "#60a5fa"}
                strokeWidth="2" strokeLinejoin="round"/>
              {/* Last point dot */}
              {vals.length > 0 && (
                <circle cx={toX(vals.length-1)} cy={toY(vals[vals.length-1])}
                  r="4" fill={view==="buy" ? "#4ade80" : "#60a5fa"}/>
              )}
            </svg>
          )}
        </div>
...
function PublicPage({ onAdminClick, onGraphClick }) {
  const [currencies, setCurrencies] = useState([]);
  ...
  async function loadData() {
    const data = await fetchCurrencies();
    setCurrencies(data);
    const lu = await fetchLastUpdated();
    setLastUpdated(lu);
  }

  // expose currencies for graph via callback
  useEffect(() => {
    if (currencies.length) window.__qasr_currencies = currencies;
  }, [currencies]);
      ...
      <header style={S.header}>
        <div style={S.hInner}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:16 }}>
            ...
            <button style={{...S.adminBtn, marginLeft:8, background:"rgba(96,165,250,0.1)", border:"1px solid rgba(96,165,250,0.3)", color:"#60a5fa"}} onClick={onGraphClick}>📈 الرسم البياني</button>
...
export default function App() {
  ...
  const [graphCurrencies, setGraphCurrencies] = useState([]);
